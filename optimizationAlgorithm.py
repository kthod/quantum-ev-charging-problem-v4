...
class optimizationAlgorithm(ABC):
    ...
    def plot_evolution(self, normalization=[], label = "") -> None:
        """
        ...
        """
        assert len(normalization) == 0 or len(normalization) == 2, "Normalization parameter must be empty or contain two elements."
        if label == "":
        ...
        plt.plot(range(maxiter), mean, linestyle='--', label=label)
